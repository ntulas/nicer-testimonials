 ...
 * @author  Matt van Andel
 */
class NT_List_table extends WP_List_Table {
	...
	public function get_columns() {
		$columns = array(
			...
			'id'    => _x( 'ID', 'Column label', 'wp-list-table-example' ),
			'name'    => _x( 'Name', 'Column label', 'wp-list-table-example' ),
			'comments'   => _x( 'Comments', 'Column label', 'wp-list-table-example' ),
			'rating' => _x( 'Rating', 'Column label', 'wp-list-table-example' ),
			'status' => _x( 'Status', 'Column label', 'wp-list-table-example' ),
		);
	...
	protected function column_default( $item, $column_name ) {
		switch ( $column_name ) {
			...
			break;
			case 'comments':
			if(strlen($item[ $column_name ]) > 120) {
				$nt_comment_pos =strpos($item[ $column_name ], ' ',120);
				return substr($item[ $column_name ],0,$nt_comment_pos);
			} else {
				return $item[ $column_name ];
			}
			break;
	...
	protected function column_name( $item ) {
		...
		$actions['edit'] = sprintf(
			'<a href="%1$s">%2$s</a>',
			esc_url( wp_nonce_url( add_query_arg( $edit_query_args, 'admin.php' ), 'editreview_' . $item['id'] ) ),
		...
		$actions['delete'] = sprintf(
			'<a class="nt-del-rev" data-ntrev-id="'.$item['id'].'"href="%1$s">%2$s</a>',
			esc_url( wp_nonce_url( add_query_arg( $delete_query_args, 'admin.php' ), 'deletereview_' . $item['id'] ) ),
			_x( 'Delete', 'List table row action', 'wp-list-table-example' )
		);

		if($item['status'] == "unapproved"){
			// Build approve row action.
			$approve_query_args = array(
				'page'   => $page,
				'action' => 'ntapprove',
				'review'  => $item['id'],
			);

			$actions['ntapprove'] = sprintf(
				'<a class="nt-app-rev" data-ntrev-id="'.$item['id'].'" href="%1$s">%2$s</a>',
				esc_url( wp_nonce_url( add_query_arg( $approve_query_args, 'admin.php' ), 'approvereview_' . $item['id'] ) ),
				_x( 'Approve', 'List table row action', 'wp-list-table-example' )
			);
		}

		if($item['status'] == "approved"){
			// Build approve row action.
			$unapprove_query_args = array(
				'page'   => $page,
				'action' => 'ntunapprove',
				'review'  => $item['id'],
			);

			$actions['ntunapprove'] = sprintf(
				'<a class="nt-unapp-rev" data-ntrev-id="'.$item['id'].'" href="%1$s">%2$s</a>',
				esc_url( wp_nonce_url( add_query_arg( $unapprove_query_args, 'admin.php' ), 'unapprovereview_' . $item['id'] ) ),
				_x( 'Unapprove', 'List table row action', 'wp-list-table-example' )
			);
		}
	...
	protected function get_bulk_actions() {
		$actions = array(
			...
			'ntapprove' => _x( 'Approve', 'List table bulk action', 'wp-list-table-example' ),
		);
	...
	protected function process_bulk_action() {
		...
		switch ($action) {
			...
			case 'ntapprove':
				$wpdb->update($wpdb->prefix.'nicertestimonials',['status' => 'approved'],['id' => $_GET['review']]);
				echo "Review approved";
				break;

			case 'ntunapprove':
				$wpdb->update($wpdb->prefix.'nicertestimonials',['status' => 'unapproved'],['id' => $_GET['review']]);
				echo "Review unapproved";
				break;
	...
	function prepare_items() {
		 ...
		 */
		$data = $wpdb->get_results("SELECT * FROM {$wpdb->prefix}nicertestimonials", ARRAY_A);
